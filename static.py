...
import csv
import os
crypto_or_stock = str(input("Crypto(c) or Stock(s)?"))
crypto_link = 'https://api.polygon.io/v2/aggs/ticker/X:ETHUSD/range/1/minute/2022-09-04/2022-09-04?adjusted=true&sort=desc&limit=50000&apiKey=ZwMEsRmpnXsoTtzA2y69x7stNYQ7cY3J'
stock_link = 'https://api.polygon.io/v2/aggs/ticker/TSLA/range/1/minute/2022-09-01/2022-09-03?adjusted=true&sort=desc&limit=50000&apiKey=ZwMEsRmpnXsoTtzA2y69x7stNYQ7cY3J'
good_data = []

if crypto_or_stock.upper() == 'C':
    df = requests.get(crypto_link).json()
    df = pd.json_normalize(df['results']) 
    df = df['o']
    df = df.values
if crypto_or_stock.upper() == 'S':
    df = requests.get(stock_link).json()
    df = pd.json_normalize(df['results']) 
    df = df['o']
    df = df.values
    
   
for pricepoint in df:
    good_data.append(pricepoint)
print(pricepoint)
new_val = []
prices = pd.DataFrame(df)
prices_val = prices.values
for i in range(0, len(prices_val)):
    new_val.append(float(str(prices_val[i]).lstrip('[').rstrip(']')))
...
def get_bollinger_bands(close_dataframe, rate=sma_inp):
    global loop_num, poc, sell_price, ttl_p, close_price, this_trades_profit, percent_over_list, bought, bad_buy, safe_money, enter, bad_sell, sold, sold2, bought2, sold_price2, good_above, is2_val, good_below, hist_port_value, short_price2, buy_price2, buy_price, buy_money, short_price, is2, sold_price, bought_back_price, total_profit, bad_above, bad_below
    ...
    if sold == True and is2 == is2_val:
        ...
        is2 = 0
        sold = False
        #this_trades_profit = (buy_money/short_price) * (short_price-current_price)
    ...
    if bought == True and is2 == is2_val:
        ...
        is2 = 0
        bought = False
        this_trades_profit = (buy_money/buy_price) * (current_price-buy_price)
        print
        print("This trades profit = $" + str(this_trades_profit))
...
#
    ...
    #    buy_price = current_price
#
    #if current_price < buy_price and enter == True:
    #        bad_below += 1
#
    #if current_price > short_price and enter == True:
    #        bad_above += 1

    #
    #percent_over = 1 - (buy_price/current_price)
    #print("percent below buy = %" + str(percent_over))
    #if percent_over <= .02:
    #    poc+=1
    #    percent_over_list.append(this_trades_profit)
    #print('poc = ', str(poc))
    #print(percent_over_list)
    #print(ttl_p)
...
#second bbbabd func
    ...
    #if buy_money > 10000:
    #    safe_money += (buy_money - 10000)
    #    buy_money = buy_money - (buy_money-10000)
    #print("Max profit: " + str(safe_money))
